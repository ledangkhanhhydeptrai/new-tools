...
# ============================================================
# PROMOTE VERIFIED MAPS METADATA
# ============================================================


def _promote_verified_metadata(
    df,
    index,
    title_col,
    address_col,
    ...
    """
    Make Maps the canonical row metadata only after search.py has verified
    the Place.

    - Strong title -> use the address returned by that exact Place.
    - Strong address -> use the title returned by that exact Place.
    - Never replace a non-empty value with an empty Maps value.
    """
    if not isinstance(result, dict) or not result.get("success"):
        return

    maps_title = safe_text(result.get("title"))
    maps_address = safe_text(result.get("address"))

    title_verified = bool(result.get("title_verified"))
    address_verified = bool(result.get("address_verified"))

    if maps_address and title_verified:
        df.at[index, address_col] = maps_address

    if maps_title and address_verified:
        df.at[index, title_col] = maps_title

    # When both are verified, canonicalize both sides.
    if title_verified and address_verified:
        if maps_title:
            df.at[index, title_col] = maps_title
        if maps_address:
            df.at[index, address_col] = maps_address
    ...
    if logger:
        logger.info(
            "PROMOTE VERIFIED MAPS METADATA | "
            "title_verified=%s | address_verified=%s | "
            "maps_title=%r | maps_address=%r",
            title_verified,
            address_verified,
            maps_title,
            maps_address,
...
def process_excel(
    ...
    # DTYPE SAFETY
    # ========================================================
    # pandas 2.x/3.x can reject writing strings/bools into columns inferred
    # as float/string extension dtypes. These columns are mutable pipeline
    # outputs, so keep them as Python object columns.
    for mutable_col in {
        title_col,
        address_col,
        url_col,
        ...
        if mutable_col in df.columns:
            df[mutable_col] = df[mutable_col].astype("object")
                ...
                if verified:
                    ...
                    if good_url and is_google_maps_place_url(good_url):
                        ...
                        if wrote:
                            ...
                            print("⚡ EXISTING GOOGLE MAPS URL -> VALIDATED")
                            print(f"🔗 {good_url}")
                            continue
